...
namespace DigitsNFCToolkit
{
	...
	public class NFCTag: IJSONSerializable
	{
        ...
        public bool IsIsoDep()
        {
            bool resp = false;
            for (int i = 0; i < this.technologies.Length && !resp; i++)
            {
                resp = resp || this.technologies[i] == NFCTechnology.ISO_DEP;
            }
            return resp;
        }
        ...
        public bool IsNfcB()
        {
            ...
            for (int i = 0; i < this.technologies.Length && !resp; i++)
            {
                resp = resp || this.technologies[i] == NFCTechnology.NFC_B;
            }
            return resp;
        }
	}
